...
; The per-round work (copy + sort of the same immutable input) is loop-invariant:
; the sorted array, and therefore the median, is identical every round, and the
; per-round element v[r % n] walks the sorted array cyclically.  We therefore
; allocate, copy and sort exactly once, then compute
;   total = rounds*median + (rounds / n)*sum(sorted) + prefix_sum(sorted, rounds % n)
; using wrapping i64 arithmetic (equal, mod 2^64, to the original running sum,
; hence identical whenever the original execution was well defined).
...
entry:
  ...
  %valid = and i1 %cmp, %cmp2
  %cmp5 = icmp sgt i32 %rounds, 0
  %or.cond = and i1 %valid, %cmp5
  br i1 %or.cond, label %alloc, label %return

alloc:                                            ; preds = %entry
  %bytes = shl nuw nsw i64 %input_length, 2
  %buf = invoke noalias noundef nonnull ptr @_Znwm(i64 noundef %bytes) #7
          to label %alloc.ok unwind label %lpad0

alloc.ok:                                         ; preds = %alloc
  call void @llvm.memcpy.p0.p0.i64(ptr nonnull align 4 %buf, ptr nonnull align 4 %input, i64 %bytes, i1 false)
  %end = getelementptr inbounds nuw i8, ptr %buf, i64 %bytes
  %ctlz = tail call range(i64 0, 64) i64 @llvm.ctlz.i64(i64 %input_length, i1 true)
  %ctlz2 = shl nuw nsw i64 %ctlz, 1
  %depth = sub nuw nsw i64 126, %ctlz2
  ...
  invoke void @_ZSt16__introsort_loopIN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEElNS0_5__ops15_Iter_comp_iterIZNSt6ranges8__detail16__make_comp_projINS9_4lessESt8identityEEDaRT_RT0_EUlOSE_OSG_E_EEEvSE_SE_SG_T1_(ptr nonnull %buf, ptr nonnull %end, i64 noundef %depth, ptr nonnull %__comp.i.i, ptr nonnull %__proj.i.i)
          to label %sort2 unwind label %lpad1

sort2:                                            ; preds = %alloc.ok
  invoke void @_ZSt22__final_insertion_sortIN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEENS0_5__ops15_Iter_comp_iterIZNSt6ranges8__detail16__make_comp_projINS9_4lessESt8identityEEDaRT_RT0_EUlOSE_OSG_E_EEEvSE_SE_SG_(ptr nonnull %buf, ptr nonnull %end, ptr nonnull %__comp.i.i, ptr nonnull %__proj.i.i)
          to label %sorted unwind label %lpad1
...
sorted:                                           ; preds = %sort2
  ...
  %iseven = icmp eq i64 %parity, 0
  %mid.ptr = getelementptr inbounds nuw i32, ptr %buf, i64 %half
  %mid = load i32, ptr %mid.ptr, align 4, !tbaa !5
  %mid64 = sext i32 %mid to i64
  br i1 %iseven, label %med.even, label %med.done

med.even:                                         ; preds = %sorted
  %lo.ptr = getelementptr inbounds i8, ptr %mid.ptr, i64 -4
  %lo = load i32, ptr %lo.ptr, align 4, !tbaa !5
  %lo64 = sext i32 %lo to i64
  %pairsum = add nsw i64 %lo64, %mid64
  %avg = sdiv i64 %pairsum, 2
  br label %med.done

med.done:                                         ; preds = %sorted, %med.even
  %median = phi i64 [ %avg, %med.even ], [ %mid64, %sorted ]
  %vec.n = and i64 %input_length, -4
  %do.vec = icmp ugt i64 %input_length, 3
  br i1 %do.vec, label %vec.body, label %vec.done

vec.body:                                         ; preds = %med.done, %vec.body
  %vi = phi i64 [ %vi.next, %vec.body ], [ 0, %med.done ]
  %vacc = phi <4 x i64> [ %vacc.next, %vec.body ], [ zeroinitializer, %med.done ]
  %vp = getelementptr inbounds nuw i32, ptr %buf, i64 %vi
  %vld = load <4 x i32>, ptr %vp, align 4, !tbaa !5
  %vext = sext <4 x i32> %vld to <4 x i64>
  %vacc.next = add <4 x i64> %vacc, %vext
  %vi.next = add nuw nsw i64 %vi, 4
  %vdone = icmp eq i64 %vi.next, %vec.n
  br i1 %vdone, label %vec.end, label %vec.body

vec.end:                                          ; preds = %vec.body
  %vred = call i64 @llvm.vector.reduce.add.v4i64(<4 x i64> %vacc.next)
  br label %vec.done

vec.done:                                         ; preds = %med.done, %vec.end
  %sum.partial = phi i64 [ %vred, %vec.end ], [ 0, %med.done ]
  %tail.start = phi i64 [ %vec.n, %vec.end ], [ 0, %med.done ]
  %has.tail = icmp ult i64 %tail.start, %input_length
  br i1 %has.tail, label %tail.body, label %sum.done

tail.body:                                        ; preds = %vec.done, %tail.body
  %ti = phi i64 [ %ti.next, %tail.body ], [ %tail.start, %vec.done ]
  %tacc = phi i64 [ %tacc.next, %tail.body ], [ %sum.partial, %vec.done ]
  %tp = getelementptr inbounds nuw i32, ptr %buf, i64 %ti
  %tv = load i32, ptr %tp, align 4, !tbaa !5
  %tv64 = sext i32 %tv to i64
  %tacc.next = add i64 %tacc, %tv64
  %ti.next = add nuw nsw i64 %ti, 1
  %tdone = icmp eq i64 %ti.next, %input_length
  br i1 %tdone, label %sum.done, label %tail.body

sum.done:                                         ; preds = %vec.done, %tail.body
  %sum.all = phi i64 [ %sum.partial, %vec.done ], [ %tacc.next, %tail.body ]
  %rounds64 = zext nneg i32 %rounds to i64
  %q = udiv i64 %rounds64, %input_length
  %rem = urem i64 %rounds64, %input_length
  %has.pref = icmp eq i64 %rem, 0
  br i1 %has.pref, label %finish, label %pref.body

pref.body:                                        ; preds = %sum.done, %pref.body
  %pi = phi i64 [ %pi.next, %pref.body ], [ 0, %sum.done ]
  %pacc = phi i64 [ %pacc.next, %pref.body ], [ 0, %sum.done ]
  %pp = getelementptr inbounds nuw i32, ptr %buf, i64 %pi
  %pv = load i32, ptr %pp, align 4, !tbaa !5
  %pv64 = sext i32 %pv to i64
  %pacc.next = add i64 %pacc, %pv64
  %pi.next = add nuw nsw i64 %pi, 1
  %pdone = icmp eq i64 %pi.next, %rem
  br i1 %pdone, label %finish, label %pref.body

finish:                                           ; preds = %sum.done, %pref.body
  %prefix = phi i64 [ 0, %sum.done ], [ %pacc.next, %pref.body ]
  %med.total = mul i64 %rounds64, %median
  %full.cycles = mul i64 %q, %sum.all
  %t0 = add i64 %med.total, %full.cycles
  %total = add i64 %t0, %prefix
  call void @_ZdlPvm(ptr noundef nonnull %buf, i64 noundef %bytes) #8
  br label %return

lpad1:                                            ; preds = %alloc.ok, %sort2
...
ehcleanup:                                        ; preds = %lpad0, %lpad1
  %exn = phi { ptr, i32 } [ %lp0, %lpad0 ], [ %lp1, %lpad1 ]
  %exn.ptr = extractvalue { ptr, i32 } %exn, 0
  %catch = call ptr @__cxa_begin_catch(ptr %exn.ptr) #9
  call void @__cxa_end_catch()
  br label %return

return:                                           ; preds = %entry, %ehcleanup, %finish
  %retval.0 = phi i64 [ 0, %ehcleanup ], [ 0, %entry ], [ %total, %finish ]
  ret i64 %retval.0
}
