...
class TestBenchmarkContract(unittest.TestCase):
    ...
    def test_run_benchmarks_contract(self, mock_run):
        ...
        try:
            stdout_file = tmp_path / "benchmark_fibonacci_seq000_stdout.txt"
            ...
            result = results[0]
            self.assertIsInstance(result, BenchmarkMeasurement)
            self.assertEqual(result.returncode, 0)
            self.assertEqual(result.benchmark_name, "fibonacci")
