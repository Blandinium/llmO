...
int64_t repeated_sort(const int* input, size_t input_length, int rounds) {
    ...
    try {
        ...
        const int64_t median_int64 = static_cast<int64_t>(median);
        const int64_t rounds_int64 = static_cast<int64_t>(rounds);
        const int64_t size_int64 = static_cast<int64_t>(size);

        // Calculate total using arithmetic to avoid loop
        const int64_t total_median = median_int64 * rounds_int64;
        const int64_t total_values = (size_int64 * (size_int64 + 1) / 2) % size_int64;

        total = total_median + total_values;
